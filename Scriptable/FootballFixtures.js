// Based almost entirely on the work of https://gist.github.com/ilyichvismara/7895bb479e14a999f3cfd2c391f10597 and https://gist.github.com/kunaldua/7f39d5953c723195bfec566d6ca45ba5

const teamId = 133667; // Replace this with your team's ID — find the team page on https://www.thesportsdb.com
...
async function createWidget() {
  ...
  const imageSize = widgetSize === "medium" ? 26 : 32;
  ...
  if (widgetSize == "small") {
    ...
    let title = titleStack.addText(teamName + "'s next")
    ...
    if (e.idHomeTeam == teamId) {
      ...
      nextOpponent = e.strAwayTeam;
      theSuffix = "(H)";
    } else {
      homeImg = await getTeamImg(e.idHomeTeam)
      awayImg = teamImg
      nextOpponent = e.strHomeTeam;
      theSuffix = "(A)";
    }
    ...
    let separator = separatorStack.addText('vs')
    ...
    let opponentStack = w.addStack();
    let opponentText = opponentStack.addText(nextOpponent + " " + theSuffix);
    opponentText.font = Font.mediumSystemFont(16);
    opponentStack.addSpacer()
  ...
  const resultsUrl = "https://www.thesportsdb.com/api/v1/json/1/eventslast.php?id=" + teamId;
  req = new Request(resultsUrl);
  res = await req.loadJSON();
  let results = res.results; 
      
  events = results.slice(0,1).concat(events)
  ...
  if (widgetSize === "large") {
    ...
    let titleStack = w.addStack()
    let title = titleStack.addText(`${teamName}'s schedule`)
  ...
  for (let i = 0; i < limitedEvents.length; i++) {
    ...
    if (e.intHomeScore != null) {
      homeNameStack.size = new Size(75, 14)
      ...
      homeScoreStack.size = new Size(25, 14)
      homeScoreStack.addSpacer()
    }
    else {
    ...
    if (e.intAwayScore != null) {
      ...
      awayScoreStack.addSpacer()
      let awayScore = awayScoreStack.addText(e.intAwayScore);
      awayScore.font = Font.mediumSystemFont(12);
      awayScoreStack.size = new Size(25, 14)
      awayScoreStack.addSpacer(10)
      awayNameStackSize = 90;
    }
   else {
    awayNameStackSize = 115;
   } 
   
   // away team name
   let awayNameStack = rowStack.addStack();
   awayNameStack.addSpacer()
   awayNameStack.size = new Size(awayNameStackSize, 14)
